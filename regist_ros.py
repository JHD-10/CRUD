...
mi_cursor = bd.cursor()
mi_cursor.execute("SELECT id, nombre, edad, email FROM alumnos")
...
ventana.title("Alum")
primero=tk.Label(ventana, text="- Registro de alumnos -", bg='#383838', fg="white", font='Times 20').pack(pady=30)
# crear Table
...
for valor in resultado:
  if (ban):
    tabla.insert(parent='', index='end', id=valor[0], tag=['t1'], values=(valor[0], valor[1], valor[2], valor[3]))
    ban=False
  else:
    tabla.insert(parent='', index='end', id=valor[0], tag=['t2'], values=(valor[0], valor[1], valor[2], valor[3]))
    ban=True
...
def regresar():
    ventana.destroy()
    subprocess.call(["python", "practica.py"])
