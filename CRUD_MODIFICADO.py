...
def agregar_alumno():
    ...
    if not re.match(r"[^@]+@[^@]+\.[^@]+", email):
        messagebox.showerror("Error al agregar el alumno", "Por favor ingrese un correo electrónico válido")
        return
    #VALIDAR PUNTO
    if not re.match(r"[^.]+@[^.]+\.[^.]+", email):
        messagebox.showerror("Error al agregar el alumno", "Por favor ingrese un correo electrónico válido")
...
def edad(char):
    return char in "abcdefghijklmnñopqrstuvwxyzABCDEFGHIJKLMNÑOPQRSTVWXYZáéíóúÁÉÍÓÚ"
validatecommand = ventana.register(edad)
Label(ventana, text="Nombre:",  font="Times", bg="#383838", fg="white").grid(row=2, column=0, padx=5, pady=5)
entrada_nombre = Entry(ventana,  font="Times", bg="#383838", fg="white", validate="key", validatecommand=(validatecommand, "%S"))
entrada_nombre.grid(row=2, column=1, padx=5, pady=5)

def numero(char):
    return char in "0123456789"
validatecommand = ventana.register(numero)
Label(ventana, text="Edad:",  font="Times", bg="#383838", fg="white").grid(row=3, column=0, padx=5, pady=5)
entrada_edad = Entry(ventana,  font="Times", bg="#383838", fg="white", validate="key", validatecommand=(validatecommand, "%S"))
...
def lista_registros():
    ventana.destroy()
    subprocess.call(["python", "alum.py"])
